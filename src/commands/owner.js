...
async function clearcf(ctx) {
  ...
  if (!cf) return send(sock, from, msg, `Confession \`${args[0]}\` not found.`);
  db.removeConfession(cf.id);
  await send(sock, from, msg, `Confession \`${cf.id}\` deleted.`);
}
...
async function autoviewstatus(ctx) {
  ...
  if (!['on', 'off'].includes(arg)) return send(sock, from, msg, 'Usage: .autoviewstatus on/off');
...
async function autoreactstatus(ctx) {
  ...
  if (!['on', 'off'].includes(arg)) {
    ...
    return send(sock, from, msg,
      ...
      `• .autoreactstatus on — enable with specific emoji\n` +
...
async function aichat(ctx) {
  ...
  if (sub === 'add') {
    ...
    if (!persona) return send(sock, from, msg, 'Set your persona first with .aichat persona <about you>');
  ...
  if (sub === 'clear') {
    ...
    db.setBotSetting('aiChatbot', false);
    return send(sock, from, msg, 'All AI chat targets cleared. AI chatbot is now OFF.');
  }
...
async function removebc(ctx) {
  ...
  const num  = jid.split('@')[0];
  await send(sock, from, msg,
    `*+${num}* removed from broadcast list.\nRemaining: *${list.length}*`);
}
...
async function clearbc(ctx) {
  ...
  db.clearBroadcast();
  await send(sock, from, msg, `Broadcast list cleared. *${list.length}* contact(s) removed.`);
}
...
async function dmgroup(ctx) {
  ...
  await send(sock, from, msg,
    ...
    `Estimated time: ~${members.length} min\n` +
...
async function clearstatusstats(ctx) {
  ...
  await send(sock, from, msg, '*Status react analytics cleared!*\n_Tracking starts fresh from now._');
}
...
async function disk(ctx) {
  ...
  try {
    ...
    await send(sock, from, msg,
      ...
      `*Free RAM (OS):* ${(os.freemem()/1024/1024).toFixed(0)} MB / ${(os.totalmem()/1024/1024).toFixed(0)} MB`
    );
...
async function lastseen(ctx) {
  ...
  if (!options[val]) return send(sock, from, msg, '*Last Seen Privacy*\nUsage: .lastseen <value>\nOptions:\n• `all` — everyone\n• `contacts` — contacts only\n• `none` — no one\nExample: .lastseen none');
  try {
...
async function ppprivacy(ctx) {
  ...
  if (!isOwnerCheck(ctx)) return send(sock, from, msg, 'Owner only!');
  const val = args[0]?.toLowerCase();
  if (!['all','contacts','none'].includes(val)) return send(sock, from, msg, '*Profile Photo Privacy*\nUsage: .ppprivacy <all/contacts/none>');
  try {
...
async function setprofilepic(ctx) {
  ...
  if (!isOwnerCheck(ctx)) return send(sock, from, msg, 'Owner only!');
  const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
  if (!quoted?.imageMessage) return send(sock, from, msg, '*Set Profile Photo*\nReply to an image with `.setprofilepic`');
  try {
    const qCtx = msg.message.extendedTextMessage.contextInfo;
    const fakeMsg = { key: { remoteJid: from, id: qCtx.stanzaId, fromMe: false, participant: qCtx.participant }, message: quoted };
    const stream = await sock.downloadMediaMessage(fakeMsg);
...
function mkToggle(key, label) {
  return async function(ctx) {
    ...
    if (!['on','off'].includes(val)) {
      ...
      return send(sock, from, msg, `*${label}*\nCurrent: *${current ? 'ON ' : 'OFF '}*\nUsage: .${ctx.command} on/off`);
    }
