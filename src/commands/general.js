...
async function menu(ctx) {
  ...
  const sections = [
    ...
    },
    {
      ...
      rows: [
        ...
        row(`${p}hexencode`,  'Hex encode text',                  `${p}hexencode`),
        row(`${p}hexdecode`,  'Hex decode text',                  `${p}hexdecode`),
        row(`${p}cipher`,     'Encrypt/decrypt text',             `${p}cipher`),
        row(`${p}rot47`,      'ROT47 cipher',                     `${p}rot47`),
        row(`${p}sslcheck`,   'Check SSL certificate',            `${p}sslcheck`),
        row(`${p}headers`,    'Scan HTTP security headers',       `${p}headers`),
        row(`${p}subdomains`, 'Find subdomains',                  `${p}subdomains`),
        row(`${p}portinfo`,   'Network port info',                `${p}portinfo`),
        row(`${p}urlinfo`,    'Trace URL redirects',              `${p}urlinfo`),
        row(`${p}macinfo`,    'MAC address vendor lookup',        `${p}macinfo`),
        row(`${p}numlookup`,  'Phone number lookup',              `${p}numlookup`),
        row(`${p}reversedns`, 'Reverse DNS lookup',               `${p}reversedns`),
        row(`${p}robotstxt`,  'Fetch robots.txt',                 `${p}robotstxt`),
        row(`${p}sitestatus`, 'Check if a site is up',            `${p}sitestatus`),
        row(`${p}breach`,     'Check email breach (HIBP)',        `${p}breach`),
        row(`${p}cvelookup`,  'Look up a CVE vulnerability',      `${p}cvelookup`),
        row(`${p}userscan`,   'Find username across platforms',   `${p}userscan`),
        row(`${p}jwtdecode`,  'Decode a JWT token',               `${p}jwtdecode`),
        row(`${p}scamalyze`,  'Detect scam messages',             `${p}scamalyze`),
        row(`${p}fakecall`,   'Fake incoming call simulation',    `${p}fakecall`),
  ...
  // Send menu image with full command list as card
  try {
