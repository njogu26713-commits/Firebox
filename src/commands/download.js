...
async function image(ctx) {
  ...
  try {
    ...
    await sock.sendMessage(from, { image: Buffer.from(res.data), caption: `*${text}*`, mimetype: 'image/jpeg' }, { quoted: msg });
...
async function gdrive(ctx) {
  ...
  await send(sock, from, msg,
    ...
    `_For files > 100MB, Google may show a warning page — click "Download anyway"._`
  );
}
...
async function gitclone(ctx) {
  ...
  try {
    ...
    const reply =
      ...
      `*Watchers:* ${d.watchers_count?.toLocaleString()}\n` +
...
async function wallpaper(ctx) {
  ...
  await send(sock, from, msg, `Fetching wallpaper: *${query}*...`);
  try {
    ...
    const res = await axios.get(url, { responseType: 'arraybuffer', timeout: 45000 });
    await sock.sendMessage(from, { image: Buffer.from(res.data), caption: `*Wallpaper: ${query}*`, mimetype: 'image/jpeg' }, { quoted: msg });
