...
async function send(sock, from, msg, text, title) {
  return sendFireboxCard(sock, from, msg, { title: title || 'Firebox Tools', content: text });
}
...
async function getpp(ctx) {
  ...
  if (!target) {
    return send(sock, from, msg,
      '*Usage:* `.getpp @user` or reply to their message with `.getpp`');
  }
...
async function rps(ctx) {
  ...
  if (!choices.includes(player)) return send(sock, from, msg, 'Usage: .rps <rock/paper/scissors>');
  ...
  await send(sock, from, msg, `*Rock Paper Scissors*\nYou: ${emojis[player]} ${player}\nBot: ${emojis[bot]} ${bot}\n${result}`);
}
